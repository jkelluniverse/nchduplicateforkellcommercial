...
const values: unknown[][] = [
  ["  DAD'S PORTFOLIO"], // banner row — skipped
  ["", "Property Address", "Tenant / Contact", "Rent ($)"], // header — skipped
...
const snap = parseTrackerRows(values, 6, 2026, asOf);
...
// Assertions
const errs: string[] = [];
if (snap.rows.length !== 5) errs.push(`row count ${snap.rows.length} != 5`);
